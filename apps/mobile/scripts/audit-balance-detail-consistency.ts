...
import { getPaymentMethodValueMode, isBankAccountMethod } from '../src/domains/balance/services/paymentMethodModel'
...
const sumEntriesForDisplay = (method: PaymentMethod, detailEntries: Entry[]) => {
  const valueMode = getPaymentMethodValueMode(method)
...
const flattenVisibleNodes = (nodes: Array<{ methodId: string; methodName: string; amount: number; children: any[] }>) => {
  const flat: Array<{ methodId: string; methodName: string; amount: number }> = []
  const walk = (node: { methodId: string; methodName: string; amount: number; children: any[] }) => {
...
months.forEach((ym) => {
  const month = dayjs(`${ym}-01`)
  const overview = buildBalanceOverview(entries, paymentMethods, recurringRules, month)

  overview.bankSummaries.forEach((summary) => {
    const method = paymentMap.get(summary.bankMethodId)
    if (!method || !isBankAccountMethod(method)) return
    const detailAmount = sumEntriesForDisplay(method, summary.detailEntries)
    if (detailAmount !== summary.detailTotal) {
      mismatches.push(
        `${ym} / ${summary.bankName}: 画面=${summary.detailTotal}, 明細合計=${detailAmount}`
      )
    }
  })
  ...
  visibleMethods.forEach((item) => {
    ...
    if (!method) return
    const detailEntries = buildMethodDetailEntriesForMonth(item.methodId, entries, paymentMethods, recurringRules, month)
    const detailAmount = sumEntriesForDisplay(method, detailEntries)
    if (detailAmount !== item.amount) {
      mismatches.push(
        `${ym} / ${item.methodName}: 画面=${item.amount}, 明細合計=${detailAmount}`
      )
    }
  })
})
